var data = (localStorage.getItem('subjectList')) ? JSON.parse(localStorage.getItem('subjectList')):{
    subject: [],
    completed: []
  };
  ...
  // If there is any text inside the item field, add that text to the todoy list
  ...
  function addItem (value) {
    ...
    data.subject.push(value);
    dataObjectUpdated();
  }
  
  function rendersubjectList() {
    if (!data.subject.length && !data.completed.length) return;
  
    for (var i = 0; i < data.subject.length; i++) {
      var value = data.subject[i];
      addItemToDOM(value);
    }
  ...
  function removeItem() {
    ...
    if (id === 'subject') {
      data.subject.splice(data.subject.indexOf(value), 1);
    } else {
  ...
  function completeItem() {
    ...
    var target = (id === 'subject') ? document.getElementById('complete'):document.getElementById('subject');
  
    parent.removeChild(item);
    target.insertBefore(item, target.childNodes[0]);
  }
  
  // Add a new item to the subject list
  function addItemToDOM(text, completed) {
    var list = (completed) ? document.getElementById('complete'):document.getElementById('subject');
